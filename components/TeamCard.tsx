...
const TeamCard = ({ image, name, title, socials }: TeamCardProps) => {
    return (
        <div className="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 p-4">
            <div className="bg-[#f7f5ec] text-center relative overflow-hidden rounded-xl shadow-md p-6 group">
                <div className="relative w-32 h-32 mx-auto mb-12">
                    <div className="absolute inset-0 rounded-full bg-blue-600 opacity-90 scale-125 transition-all duration-300 ease-linear group-hover:h-full"></div>
                    <img
                        className="relative w-full h-full object-cover rounded-full transition-transform duration-700 ease-in-out group-hover:scale-75 group-hover:shadow-[0_0_0_14px_#f7f5ec]"
                        src={image}
                        alt={name}
                    />
                </div>
                ...
                <ul className="absolute bottom-[-100px] left-0 w-full bg-blue-600 flex justify-center space-x-4 py-2 transition-all duration-500 ease-in-out group-hover:bottom-0">
                    {socials.map((social, index) => (
                        <li key={index}>
                            <a
                                href={social.link}
                                className={`${social.icon} text-white text-lg p-2 transition-colors duration-300 hover:text-blue-600 hover:bg-[#f7f5ec] rounded-full`}
                                aria-hidden="true"
                            ></a>
                        </li>
                    ))}
